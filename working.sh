      ...
      if grep -q "^${root_property}:" "$CASSANDRA_CONFIG"; then
        if grep -q "^    # ${nested_property}:" "$CASSANDRA_CONFIG"; then
         # Check if the nested property is commented out
          echo "commented property!!! property is: $nested_property with value $value"
         sed -i "/^${root_property}:/,/^[^ ]/ s|^\(    # ${nested_property}:\).*|    ${nested_property}: ${value}|" "$CASSANDRA_CONFIG"
        elif grep -q "^    ${nested_property}:" "$CASSANDRA_CONFIG"; then
